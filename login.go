...
// 职责：
//   - 打开有头浏览器，让用户手动完成登录
//   - 支持验证码、双因素认证等需要人工交互的场景
//   - 登录完成后自动保存 Cookie 到 .instagram_session.json
//
// 流程：
//   1. 创建有头浏览器上下文（headless=false，显示 UI）
//   2. 导航到 Instagram 登录页
//   3. 等待用户手动输入账号密码、完成验证码等
//   4. 用户按回车键后，从浏览器读取 Cookie
//   5. 保存 Cookie 到文件
//   6. 关闭浏览器
//
// 关键函数：
//   - ManualLogin()：主入口，执行完整的登录流程
//
// ============================================================================
