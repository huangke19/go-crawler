...
//
// 核心概念：
//   - 全局 HTTP 客户端：复用连接，减少握手开销
//   - 并发控制：使用 channel 信号量限制并发数
//   - 重试机制：网络失败时自动重试，写入失败时删除半成品文件
//   - 文件命名规则：
//     * 单图：post_<index>.jpg
//     * 单视频：post_<index>.mp4
...
//
// 关键函数：
//   - CreateUserDirectory()：创建 downloads/<username>/ 目录
//   - DownloadMedia()：下载单个文件，支持重试
//   - DownloadPost()：下载帖子的所有媒体
//   - DownloadPostAndReturnPaths()：下载并返回文件路径列表
//   - downloadPostInternal()：内部下载函数
//   - downloadMediaByShortcode()：按 shortcode 下载（用于缓存模式）
